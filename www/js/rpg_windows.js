...
function Window_ReviewList() {
  this.initialize.apply(this, arguments);
}

Window_ReviewList.prototype = Object.create(Window_Selectable.prototype);
Window_ReviewList.prototype.constructor = Window_ReviewList;

Window_ReviewList.prototype.initialize = function (x, y, width, height) {
  Window_Selectable.prototype.initialize.call(this, x, y, width, height);
  this.activate();
  this._mode = null;
};

Window_ReviewList.prototype.setMode = function (mode) {
  this._mode = mode;
};

Window_ReviewList.prototype.maxItems = function () {
  return DataManager.maxSavefiles();
};

Window_ReviewList.prototype.maxVisibleItems = function () {
  return 5;
};

Window_ReviewList.prototype.itemHeight = function () {
  var innerHeight = this.height - this.padding * 2;
  return Math.floor(innerHeight / this.maxVisibleItems());
};

Window_ReviewList.prototype.drawItem = function (index) {
  var id = index + 1;
  var valid = DataManager.isThisGameFile(id);
  var info = DataManager.loadSavefileInfo(id);
  var rect = this.itemRectForText(index);
...
Window_ReviewList.prototype.drawFileId = function (id, x, y) {
  this.drawText(TextManager.file + " " + id, x, y, 180);
};

Window_ReviewList.prototype.drawContents = function (info, rect, valid) {
  var bottom = rect.y + rect.height;
  if (rect.width >= 420) {
    this.drawGameTitle(info, rect.x + 192, rect.y, rect.width - 192);
    if (valid) {
...
Window_ReviewList.prototype.drawGameTitle = function (info, x, y, width) {
  if (info.title) {
    this.drawText(info.title, x, y, width);
  }
};

Window_ReviewList.prototype.drawPartyCharacters = function (info, x, y) {
  if (info.characters) {
    for (var i = 0; i < info.characters.length; i++) {
      var data = info.characters[i];
      this.drawCharacter(data[0], data[1], x + i * 48, y);
...
Window_ReviewList.prototype.drawPlaytime = function (info, x, y, width) {
  if (info.playtime) {
    this.drawText(info.playtime, x, y, width, "right");
  }
};

Window_ReviewList.prototype.playOkSound = function () {};
