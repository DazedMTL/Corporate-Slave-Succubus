  ...
  SE.parseParameters = function (string) {
    try {
      return JSON.parse(string, (key, value) => {
        try {
          return SE.parseParameters(value);
        } catch (e) {
          return value;
        }
      });
...
(function () {
  ...
  for (let key in SE.Params.SkipTitle) {
    SE.Params.SkipTitle[key] = SE.Params.SkipTitle[key].replace("\r", "");
  } // Because: fix stupid RMMV bug (https://forums.rpgmakerweb.com/index.php?threads/parameter-string-does-not-equal-string.113697/)
...
(function () {
  function bShowTitleScreen() {
    return (
      ...
      SE.Params.SkipTitle.SkipIfSavefile === false
    );
  }
  ...
  /*╔══════════════════════════════════════╗
      ...
      ╚══════════════════════════════════════╝*/
  SE.Alias.SkipTitleScreen_Scene_Boot_prototype_start =
    Scene_Boot.prototype.start;
  Scene_Boot.prototype.start = function () {
    if (bShowTitleScreen() === true) {
      // Load regular titlescreen.
      SE.Alias.SkipTitleScreen_Scene_Boot_prototype_start.apply(
        this,
        arguments
      );
    } // Skip titlescreen.
  ...
  SceneManager.goto = function (sceneClass) {
    if (SE.SkipTitle.SkipTitleEntirely === true && sceneClass === Scene_Title) {
      SE.Alias.SkipTitleScreen_SceneManager_goto.call(
        this,
        eval(SE.SkipTitle.StartSceneAsStr)
      );
    } else {
