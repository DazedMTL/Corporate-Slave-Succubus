...
Felski.AUTOSAVE.autosaveText = String(
  parameters["Auto Save Text"] || "Autosave"
);
Felski.AUTOSAVE.saveCounter = 1;

Felski.AUTOSAVE.onMapChange = String(
  parameters["Save After Map Change"] || "true"
);
Felski.AUTOSAVE.onMapChange = eval(Felski.AUTOSAVE.onMapChange);
Felski.AUTOSAVE.onMapChangeText = String(
  parameters["Save After Map Change Text"] || "Entering Area"
);
...
Felski.AUTOSAVE.onMenuExitText = String(
  parameters["Save On Menu Exit Text"] || "Closing Menu"
);
Felski.AUTOSAVE.standardText = String(
  parameters["Save Standard Text"] || "Normal Save"
);

Felski.AUTOSAVE.enableAutosave = String(
  parameters["Enable Auto Saving"] || "true"
);
Felski.AUTOSAVE.enableAutosave = eval(Felski.AUTOSAVE.enableAutosave);

Felski.AUTOSAVE.enableChangeSaveTitles = String(
  parameters["Change Save Titles"] || "false"
);
Felski.AUTOSAVE.enableChangeSaveTitles = eval(
  Felski.AUTOSAVE.enableChangeSaveTitles
);
...
(function () {
  ...
  var ZERO_Scene_File_prototype_createListWindow =
  ...
  var ZERO_DataManager_saveGameWithoutRescue =
  ...
  Game_System.prototype.autoSaveGame = function () {
    ...
    if (DataManager.saveGame(Felski.AUTOSAVE.saveCounter)) {
      console.log(
        "Autosave successful. Saved in slot " +
          Felski.AUTOSAVE.saveCounter +
          " with trigger " +
          Felski.AUTOSAVE.triggerText
      );
  ...
  Felski.AUTOSAVE.DataManager_extractSaveContents =
  ...
  DataManager.makeSavefileInfo = function () {
    ...
    if (Felski.AUTOSAVE.enableChangeSaveTitles)
  ...
  Window_SavefileList.prototype.drawFileId = function (id, x, y) {
    if (id <= Felski.AUTOSAVE.slots) {
      ...
      this.drawText(
        TextManager.file + " " + (id - Felski.AUTOSAVE.slots),
        x,
        y,
        180
      );
  ...
  Felski.AUTOSAVE.Game_Player_performTransfer =
  ...
  // Transfer Player
  Felski.AUTOSAVE.Game_Interpreter_command201 =
  ...
  Felski.AUTOSAVE.Game_Interpreter_pluginCommand =
  ...
  if (Imported.YEP_SaveCore) {
    ...
    Felski.AUTOSAVE.Window_SaveAction_isSaveEnabled =
      Window_SaveAction.prototype.isSaveEnabled;
