...
(function () {
  ...
  for (var p of pKey) {
    ...
    if (key_ids[p]) {
      // if key is listed in key_ids
      if (!Input.keyMapper[key_ids[p]]) {
        // if key isn't already registered with keyMapper
        Input.keyMapper[key_ids[p]] = p; // add it to keyMapper, with label from key_ids
      }
    } else {
      if (!Input.keyMapper[p]) {
        // if key is not already registered with keyMapper and also not in key_ids
  ...
  Window_Message.prototype.update = function () {
    ...
    $gameSwitches.setValue(Galv.MBG.s, !MessageHide_messageWindowVisible);
    this.opacity = 0;
  };
