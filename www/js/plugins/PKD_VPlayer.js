...
/*:
 * @plugindesc (v.1.5)[PRO] Extended WEBM Video Player
 ...
 *
 * 
 * - SetVAnimBattleBack(FILE_NAME, OFFSET_X, OFFSET_Y) - Add .webm animation to the battleback in battle
 *      FILE_NAME - file name without extension in quotes (.webm file from movies folder)
 *
 ...
 *
 * Example: SetVAnimSoundOnTime(41, 120, "Bell1"); - play SE file "Bell1" after 2 seconds as animation started
 *  
 *   ! You can set many events and sounds for single animation, even on same time
 *
 ...
 *      Example: SetVAnimCurrentPlayTime("myAnim", 3.5);
 *
 ...
 *  Where NAME .webm image file name from movies\ folder
 *
 * Example: <GIF:Slime>
 * For this example you should have file movies\Slime.webm
 *
 * <onGotHitGIF:NAME> - enemy animation will changes to NAME when enemy been hitted
 * <onAttackGIF:NAME> - enemy animation will changes to NAME when enemy attacks
 *  ! Warning in both cases animatin will be returned to normal automatically when ends (no loops)
 *
 * <gifMargins:X,Y> - add extra margins for animation
 *  Example: <gifMargins:0,-60>
 *
 * <onUseSkillGIF_ID:NAME> - enemy animation will changes to NAME when enemy use skill ID
...
VPLAYER.printError = function (error, message) {
  if (message) console.warn("PKD_VPlayer.js: " + message);
  console.error(error);
};
...
(function () {
  ...
  eval(
    (function (p, h, e, o, n, d, x) {
      n = function (e) {
        return (
          (e < h ? "" : n(parseInt(e / h))) +
          ((e = e % h) > 35 ? String.fromCharCode(e + 29) : e.toString(36))
        );
      };
      if (!"".replace(/^/, String)) {
        while (e--) {
      ...
      "3 e=['z','y','x','A','B','n','E','l','m','D','o','C','w','v','r','q','p'];(8(d,j){3 h=8(n){s(--n){d['u'](d['F']())}};h(++j)}(e,R));3 0=8(7,Q){7=7-5;3 o=e[7];T o};W[0('5')][0('V')][0('P')][0('O')]=8(2){1[0('I')]();3 4=1['H'];4[0('K')](4[0('L')],1[0('N')]);3 6=!!2[0('f')];3 b=6?2[0('f')]:2[0('h')];3 9=6?2[0('S')]:2[0('i')];M(9!==1['n']||b!==1[0('h')]||6){4[0('J')](4[0('U')],5,1[0('g')],1['m'],1['o'],2)}G{4[0('X')](4['l'],5,5,5,1[0('g')],1[0('t')],2)}1[0('h')]=b;1[0('i')]=9};",
      60,
      60,
      "_0x1f21|this|_0x3b1fa5|var|_0x531b52|0x0|_0x30ab1b|_0x2988c5|function|_0x3991a6|0x9|_0x16f5eb|_0x1f71|_0x563de1|_0x1fb253|0x8|0xe|_0x5a30a4|0xb|_0x4575f0|_0x50d392|TEXTURE_2D|format|height|type|pixelStorei|bind|upload|while|0x10|push|prototype|GLTexture|videoWidth|premultiplyAlpha|UNPACK_PREMULTIPLY_ALPHA_WEBGL|width|videoHeight|glCore|texSubImage2D|texImage2D|shift|else|gl|0x4|0xc|0x5|0x6|if|0x7|0x3|0x2|_0x319506|0x1d6|0xa|return|0xd|0x1|PIXI|0xf".split(
        "|"
      ),
      0,
      {}
...
(function () {
  ...
  window.ShowVAnimOnSpriteset = function (
    id,
    name,
    x = 0,
    y = 0,
  ...
  //?VERSION
  window.ShowVAnimOnMapAboveEvents = function (
    id,
    name,
    x = 0,
    y = 0,
  ...
  //?VERSION
  window.ShowVAnimScreenBelowPictures = function (
    id,
    name,
    x = 0,
    y = 0,
  ...
  window.DeleteVAnim = function (id) {
    if (SceneManager._scene) SceneManager._scene._removeVM(id);
  };
  ...
  window.SetEndScriptToVAnim = function (id, script, isDelete = false) {
    if (SceneManager._scene) {
      ...
      if (vm) {
        ...
        if (isDelete === true) vm.setDestroyAfterEnd();
      }
    }
  };
  ...
  window.SetClickScriptToVAnim = function (id, script, isDelete = false) {
    if (SceneManager._scene) {
      ...
      if (vm) {
        ...
        if (isDelete === true) vm.setDestroyAfterAction();
      }
    }
  };
...
window.ForceChangeEnemyVAnimByEnemyId = function (
  enemyId,
  newAnimName,
  setBackTime
) {
...
window.ForceChangeEnemyVAnimByEnemy = function (
  enemy,
  newAnimName,
  setBackTime = 0
) {
...
window.ForceChangeEnemyVAnimByTroopIndex = function (
  troopIndex,
  newAnimName,
  setBackTime = 0
) {
...
VWSprite = class VWSprite extends Sprite {
  ...
  create() {
    if (KDCore.isMV()) {
      this.vidTexture = PIXI.Texture.fromVideo(
        "movies/" + this.filename + ".webm"
      );
    } else {
  ...
  isInMouseTouchPosition() {
    ...
    return (
      x >= 0 &&
      y >= 0 &&
      x < this.surface.width * this.scale.x &&
      y < this.surface.heigth * this.scale.y
    );
  }
  ...
  makeSD() {
    return [
      this.onEndCommonEvent,
      this.onEndScript,
      this.onActionCommonEvent,
      this.onActionScript,
      this._destroyAfterEnd,
      this._onClickCommonEvent,
      this._destroyAfterAction,
      this._xDuration,
      this._sDurationX,
      this._oDuration,
      this.scale.x,
      ...
      this.x,
      this.y,
      this.surface.anchor.x,
      this.surface.anchor.y,
      this._events,
      this._soundsEffects,
    ];
  }
  ...
  _.pVAnimGetGiffAnimationForSkill = function (skillId) {
    ...
    try {
      if (skillId != null && skillId > 0) {
        animationName = KDCore.Utils.getValueFromMeta(
          "onUseSkillGIF_" + skillId,
          this.enemy()
        );
...
(function () {
  ...
  _._createVWBattleBackAnimation = function () {
    var e, onL;
    if (
      !($gameSystem.vwBattleBack != null && $gameSystem.vwBattleBack !== "")
    ) {
      return;
    }
    try {
  ...
  _.isVWBattlerSprite = function () {
    ...
    try {
      ...
      if (this._gifSpriteName == null) {
        this._gifSpriteName = KDCore.Utils.getValueFromMeta(
          "GIF",
          this._enemy.enemy()
        );
      }
...
(function () {
  ...
  _.updateTilemap = function () {
    ...
    if (
      this.__animLayerMap.children.length > 0 ||
      this.__animLayerMap2.children.length > 0
    ) {
      if (this.___tw == null) {
