...
(function () {
  ...
  PluginManager.requiredPlugin = function (name, required, version) {
    if (!VictorEngine.BasicModule) {
      ...
      VictorEngine.FogAndOverlay.requiredPlugin.call(
        this,
        name,
        required,
        version
...
(function () {
  ...
  if (Imported["VE - Basic Module"]) {
    ...
    VictorEngine.Parameters.FogAndOverlay.MaxFogs =
      Number(parameters["Max Fogs"]) || 1;
  }
  ...
  Game_Screen.prototype.showFog = function (
    fogId,
    name,
    opacity,
    hue,
  ...
  Game_Map.prototype.setupFog = function (note) {
    var name = String(
      this.fogName(note, new RegExp("name:[ ]*('[^']+'|\"[^\"]+\")", "gi"), "")
    );
    var id = Number(this.fogValue(note, new RegExp("id:[ ]*(\\d+)", "gi"), 0));
    var op = Number(
      this.fogValue(note, new RegExp("opacity:[ ]*(\\d+)", "gi"), 192)
    );
    var zoom = Number(
      this.fogValue(note, new RegExp("zoom:[ ]*(\\d+)%?", "gi"), 100)
    );
    var hue = Number(
      this.fogValue(note, new RegExp("hue:[ ]*(\\d+)", "gi"), 0)
    );
    var blend = Number(
      this.fogValue(note, new RegExp("blend:[ ]*(\\d+)", "gi"), 0)
    );
    var moveX = Number(
      this.fogValue(note, new RegExp("move x:[ ]*([+-]?\\d+)", "gi"), 0)
    );
    var moveY = Number(
      this.fogValue(note, new RegExp("move y:[ ]*([+-]?\\d+)", "gi"), 0)
    );
    var panX = Number(
      this.fogValue(note, new RegExp("pan x:[ ]*(\\d+)", "gi"), 0)
    );
    var panY = Number(
      this.fogValue(note, new RegExp("pan y:[ ]*(\\d+)", "gi"), 0)
    );
    var depth = Number(
      this.fogValue(note, new RegExp("depth:[ ]*(\\d+)", "gi"), 1)
    );
  ...
  VictorEngine.FogAndOverlay.pluginCommand =
  ...
  VictorEngine.FogAndOverlay.createLowerLayer =
  ...
  VictorEngine.FogAndOverlay.createBattleback =
  ...
  VictorEngine.FogAndOverlay.updateSpritesetBattle =
